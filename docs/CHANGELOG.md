...
## [Unreleased]
- [修复] 打通“全市场双源行情→历史与交易计划→千问/DeepSeek一致复核→盘中新鲜买入区→2.5%模拟建仓建议”闭环，新增逐层候选漏斗、零输入/模型链路红灯与去重恢复告警；PRIMARY 现金达到25%后抑制常规止盈减仓但保留硬止损，建仓扣除待执行买入预留后仍须保留至少15%现金，避免策略单向现金化或反向耗尽现金。
