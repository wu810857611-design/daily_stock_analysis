...
双模型结构化复核各自按最多4只一批请求同一个已配置模型，保留每只完整的输入证据。
每批单次请求最长120秒，超时、连接失败、408/429及临时5xx最多尝试2次，重试前等待
2秒；单个模型整轮共用480秒预算，剩余预算不足时缩短请求期限，超过预算返回的结果
也会拒绝使用。SDK 内层重试关闭，避免多层重试叠加。千问不会用 DeepSeek 代替，
两者也不会调用全局备用路由；有效的 pass/watch/reject 不重试，不以重试改变既有否决。
鉴权/参数错误、无效JSON、截断、代码缺失/重复/越界均直接失败；任何一批失败都不会
把部分结果当成完整复核，也不能因此进入受限2.5%档。无论一方失败与否，另一方仍
独立复核，最终沿用原有失败告警和买入拦截。
`diagnostics.llm_calls` 保留逻辑复核次数，新增 `diagnostics.reviewer_requests` 记录每个
模型的分批请求/重试、完成数量、耗时和错误类型，不保存密钥、请求正文或原始错误体。
