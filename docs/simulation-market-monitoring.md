...
- `02-market-scan.yml`：北京时间 10:30、14:30 和 19:15 分层扫描 A 股全市场与
  ...
  DeepSeek 独立复核。A 股快照按东财、Sina 双源降级；港股通成分接口异常时，只能
  使用仍在有效期内的已验证港股通成员集合过滤新鲜全港行情，不会把非港股通标的
  混入。任一市场零输入或双模型链路中断会红灯失败、保留报告并发送去重健康告警。
...
分钟监控拿到新鲜行情且价格进入买入区，才发送首笔 2.5% 模拟净值的建仓建议，仍需
人工确认。每份扫描报告同时展示“全市场输入 → L1 → 历史/计划 → 双模型 → 可建仓”
...
策略影子账户以股票仓位 75%–85%、现金 15%–25% 为组合护栏。现金已经达到或超过
25% 时，普通目标位止盈不再继续机械减仓，而是明确提示“建议不减仓，继续持有”；
硬止损和确定性风险退出始终优先，不受现金护栏阻断。新建仓还会扣除尚未执行的买入
预留与成本，买入后现金不得低于15%。旁路观察账户没有可信的完整现金/NAV 时不会
套用 PRIMARY 现金比例，也不会把其数据写入 PRIMARY 实验。
